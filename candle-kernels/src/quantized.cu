...
// BF16-output variant of mul_mat_vec_q.
// Identical to mul_mat_vec_q but writes __nv_bfloat16 to dst, eliminating the
// separate F32→BF16 conversion kernel that follows each GEMV in the default path.
template<int ncols_y, int qk, int qi, typename block_q_t, int vdr, vec_dot_q_cuda_t vec_dot_q_cuda>
static __device__ void mul_mat_vec_q_bf16out(
...
#pragma unroll
        ...
        if (threadIdx.x < rows_per_cuda_block) {
            // Write BF16 directly — no separate F32→BF16 kernel needed.
            dst[j*nrows_dst + row0 + threadIdx.x] = __float2bfloat16(tmp[j][threadIdx.x]);
        }
    }
}

// BF16-output wrappers for batch_size=1 (the decode GEMV path).
// Named with suffix _bf16out1 to distinguish from the F32-output variants.
#define DEF_BF16OUT_KERNEL(QTYPE, QK, QI, BLOCK_T, VDR, VEC_DOT)                   \
extern "C" __global__ void mul_mat_vec_##QTYPE##_q8_1_bf16out1(                     \
    const void * vx, const void * vy, __nv_bfloat16 * dst,                         \
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) { \
    mul_mat_vec_q_bf16out<1, QK, QI, BLOCK_T, VDR, VEC_DOT>                        \
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);                       \
}

// ── Fused BF16-input + BF16-output GEMV ──────────────────────────────────────
//
// Single-kernel version of the two-kernel path (BF16→Q8_1 + GEMV→BF16).
// Quantizes the BF16 input to Q8_1 on-the-fly in shared memory, then computes
// the dot product with the Q4K weight matrix, writing BF16 to dst.
// This eliminates one kernel launch per GEMV call (277 per decode step).
//
// Template parameters match mul_mat_vec_q_bf16out.
// The input `vy` is now raw BF16 (not pre-quantized Q8_1).
template<int qk, int qi, typename block_q_t, int vdr, vec_dot_q_cuda_t vec_dot_q_cuda>
static __device__ void mul_mat_vec_q_fused_bf16in_bf16out(
    const void * __restrict__ vx, const __nv_bfloat16 * __restrict__ vy_bf16,
    ...
    const int blocks_per_row_x  = ncols_x / qk;
    const int blocks_per_col_q8 = nrows_y / QK8_1;  // nrows_y = ncols_x
    constexpr int blocks_per_iter = vdr * nwarps * WARP_SIZE / qi;

    float tmp = 0.0f;
    const block_q_t * x = (const block_q_t *) vx;

    // Shared memory to hold quantized Q8_1 blocks for the input.
    // Sized using nrows_y (the padded column count) so that K-quant vec_dot
    // accessors, which look ahead by up to QR*_K-1 blocks beyond the current
    // kby index, never read past the allocated region.
    // For ncols_x=1536, MATRIX_ROW_PADDING=512: nrows_y=2048, 64 blocks × 36B = 2304 bytes.
    extern __shared__ block_q8_1 y_q8_smem[];
    const int n_q8_blocks_real   = ncols_x / QK8_1;  // blocks holding real data
    const int n_q8_blocks_padded = nrows_y / QK8_1;  // total shared-mem slots (includes padding)

    // Phase 1: quantize BF16 input → Q8_1 into shared memory.
    // Zero-initialise the padding slots so vec_dot reads of those slots are safe.
    for (int blk = threadIdx.y; blk < n_q8_blocks_padded; blk += nwarps) {
        if (blk >= n_q8_blocks_real) {
            // Padding block: write zero so vec_dot reads are benign.
            if (threadIdx.x == 0) {
                y_q8_smem[blk].ds = __floats2half2_rn(0.0f, 0.0f);
            }
            for (int i = threadIdx.x; i < QK8_1; i += WARP_SIZE) {
                y_q8_smem[blk].qs[i] = 0;
            }
            continue;
        }
    }
    // Each warp (threadIdx.y) handles consecutive blocks.
    // Within each block, warp lane j handles element j.
    for (int blk = threadIdx.y; blk < n_q8_blocks_real; blk += nwarps) {
        const int blk_start = blk * QK8_1;
        const int lane = threadIdx.x;
        // Each lane loads one element (lane < 32 = QK8_1).
        float v = (lane < QK8_1) ? __bfloat162float(vy_bf16[blk_start + lane]) : 0.0f;
        float amax = fabsf(v);
        float sum_val = v;
        ...
        if (lane < QK8_1) {
            y_q8_smem[blk].qs[lane] = (int8_t)__float2int_rn(v * inv_d);
        }
        if (lane == 0) {
            y_q8_smem[blk].ds = __floats2half2_rn(d, sum_val);
        }
    }
    __syncthreads();

    // Phase 2: dot product using existing vec_dot (reads from shared Q8_1).
    const block_q8_1 * y_q8 = y_q8_smem;

    for (int kbx = tid / (qi/vdr); kbx < blocks_per_row_x; kbx += blocks_per_iter) {
        const int kby = kbx * (qk/QK8_1);
        const int kqs = vdr * (tid % (qi/vdr));
        tmp += vec_dot_q_cuda(&x[kbx + row0 * blocks_per_row_x], &y_q8[kby], kqs);
    }
...
// Fused BF16-in BF16-out wrappers for batch_size=1.
// Named with suffix _fused_bf16in_bf16out1.
// Shared memory: n_q8_blocks_padded * sizeof(block_q8_1) bytes, where
// n_q8_blocks_padded = ncols_padded / QK8_1 (ncols_padded passed as nrows_y).
// The padding ensures K-quant vec_dot look-ahead offsets stay in bounds.
// For ncols=1536, MATRIX_ROW_PADDING=512: 64 * 36 = 2304 bytes.
#define DEF_FUSED_BF16_KERNEL(QTYPE, QK, QI, BLOCK_T, VDR, VEC_DOT)                \
extern "C" __global__ void mul_mat_vec_##QTYPE##_fused_bf16in_bf16out1(             \
    const void * vx, const __nv_bfloat16 * vy_bf16, __nv_bfloat16 * dst,          \
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst)  \
{                                                                                    \
    mul_mat_vec_q_fused_bf16in_bf16out<QK, QI, BLOCK_T, VDR, VEC_DOT>             \
        (vx, vy_bf16, dst, ncols_x, nrows_x, nrows_y, nrows_dst);                  \
}

DEF_FUSED_BF16_KERNEL(q4_0, QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ, vec_dot_q4_0_q8_1)
DEF_FUSED_BF16_KERNEL(q4_K, QK_K,  QI4_K, block_q4_K, VDR_Q4_K_Q8_1_MMVQ, vec_dot_q4_K_q8_1)
DEF_FUSED_BF16_KERNEL(q5_K, QK_K,  QI5_K, block_q5_K, VDR_Q5_K_Q8_1_MMVQ, vec_dot_q5_K_q8_1)
DEF_FUSED_BF16_KERNEL(q6_K, QK_K,  QI6_K, block_q6_K, VDR_Q6_K_Q8_1_MMVQ, vec_dot_q6_K_q8_1)
DEF_FUSED_BF16_KERNEL(q3_K, QK_K,  QI3_K, block_q3_K, VDR_Q3_K_Q8_1_MMVQ, vec_dot_q3_K_q8_1)
DEF_FUSED_BF16_KERNEL(q2_K, QK_K,  QI2_K, block_q2_K, VDR_Q2_K_Q8_1_MMVQ, vec_dot_q2_K_q8_1)

DEF_BF16OUT_KERNEL(q4_0,  QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ, vec_dot_q4_0_q8_1)
DEF_BF16OUT_KERNEL(q4_1,  QK4_1, QI4_1, block_q4_1, VDR_Q4_1_Q8_1_MMVQ, vec_dot_q4_1_q8_1)
DEF_BF16OUT_KERNEL(q5_0,  QK5_0, QI5_0, block_q5_0, VDR_Q5_0_Q8_1_MMVQ, vec_dot_q5_0_q8_1)
DEF_BF16OUT_KERNEL(q5_1,  QK5_1, QI5_1, block_q5_1, VDR_Q5_1_Q8_1_MMVQ, vec_dot_q5_1_q8_1)
DEF_BF16OUT_KERNEL(q8_0,  QK8_0, QI8_0, block_q8_0, VDR_Q8_0_Q8_1_MMVQ, vec_dot_q8_0_q8_1)
DEF_BF16OUT_KERNEL(q2_K,  QK_K,  QI2_K, block_q2_K, VDR_Q2_K_Q8_1_MMVQ, vec_dot_q2_K_q8_1)
DEF_BF16OUT_KERNEL(q3_K,  QK_K,  QI3_K, block_q3_K, VDR_Q3_K_Q8_1_MMVQ, vec_dot_q3_K_q8_1)
DEF_BF16OUT_KERNEL(q4_K,  QK_K,  QI4_K, block_q4_K, VDR_Q4_K_Q8_1_MMVQ, vec_dot_q4_K_q8_1)
DEF_BF16OUT_KERNEL(q5_K,  QK_K,  QI5_K, block_q5_K, VDR_Q5_K_Q8_1_MMVQ, vec_dot_q5_K_q8_1)
DEF_BF16OUT_KERNEL(q6_K,  QK_K,  QI6_K, block_q6_K, VDR_Q6_K_Q8_1_MMVQ, vec_dot_q6_K_q8_1)
...
// BF16 variant: quantizes BF16 input directly to Q8_1 without a separate
// BF16→F32 conversion kernel.  Eliminates one GPU kernel launch per QLinear
// GEMV on models with BF16 activations (e.g. Gemma4, Qwen3).
extern "C" __global__ void quantize_q8_1_bf16(const __nv_bfloat16 * __restrict__ x, void * __restrict__ vy, const int kx, const int kx_padded) {
    ...
    // Convert BF16 → F32 inline (no separate kernel needed)
